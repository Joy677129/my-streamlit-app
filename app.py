...
# For slideshow parsing, attempt to import python-pptx
try:
...
st.set_page_config(page_title="Set Operations & Advanced Features (click sidebar for more options)", layout="wide")
...
if show_video:
    st.sidebar.subheader("Video Viewer Settings")
    st.subheader("Video Viewer")
    # Display all hardcoded videos from VIDEO_LIST
    for vid_info in VIDEO_LIST:
        path = vid_info.get("path")
        title = vid_info.get("title") or os.path.splitext(os.path.basename(path))[0]
        st.subheader(title)
        if os.path.exists(path):
            try:
                ...
                st.error(f"Failed to play video {path}: {e}")
        else:
...
# --- Footer ---
st.markdown("---")
st.caption("Built with ❤️ using Streamlit, Matplotlib, matplotlib-venn, python-pptx & OpenAI API")
